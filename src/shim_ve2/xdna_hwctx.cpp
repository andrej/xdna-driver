    ...
    m_log_buf(nullptr)
{
  ...
  // Query the auto-selected mem_bitmap from the driver
  query_mem_bitmap();
...
query_mem_bitmap()
{
  uint32_t mem_bitmap = 0;

  amdxdna_drm_get_array array_args = {};
  array_args.param = DRM_AMDXDNA_HWCTX_MEM_BITMAP;
  array_args.element_size = m_handle;  // Pass context_id via element_size
  array_args.num_element = 1;
  array_args.buffer = reinterpret_cast<uint64_t>(&mem_bitmap);

  try {
    m_device->get_edev()->ioctl(DRM_IOCTL_AMDXDNA_GET_ARRAY, &array_args);
    m_mem_bitmap = mem_bitmap;
    shim_debug("Queried mem_bitmap=0x%x for hwctx handle=%u", m_mem_bitmap, m_handle);
  } catch (const xrt_core::system_error& ex) {
    m_mem_bitmap = 0;
    shim_debug("Failed to query mem_bitmap (using default CMA): %s (err=%d: %s)",
               ex.what(), ex.get_code(), errno_to_str(ex.get_code()));
  }
}
