...
struct handshake {
	...
	struct {
		...
		u32 forever_mode_enabled;     // 74 - Driver sets to enable forever mode
		u32 forever_stop_request;     // 78 - Driver sets to request stop
		u32 forever_iteration;        // 7c - FW updates current iteration (read-only)
		u32 forever_last_status;      // 80 - FW updates last status (read-only)
		u32 reserved1[3]; // make sure vm (below) starts at offset 0xa0
		}
		info;
		u32 raw;
	}
